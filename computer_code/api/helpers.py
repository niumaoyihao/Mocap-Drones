...
class Cameras:
    ...
    def _camera_read(self):
        ...
        for i in range(0, self.num_cameras):
            ...
            frames[i] = cv.undistort(frames[i], self.get_camera_params(i)["intrinsic_matrix"],
                                     self.get_camera_params(i)["distortion_coef"])
        ...
        if (self.is_capturing_points):
            ...
            if (any(np.all(point[0] != [None, None]) for point in image_points)):
                if self.is_capturing_points and not self.is_triangulating_points:
                    ...
                    errors, object_points, frames = find_point_correspondance_and_object_points(image_points,
                                                                                                self.camera_poses,
                                                                                                frames)
                        ...
                        if len(filtered_objects) != 0:
                            for filtered_object in filtered_objects:
                                if self.drone_armed[filtered_object['droneIndex']]:
                                    ...
                                    serial_data = {
                                        "pos": [round(x, 4) for x in filtered_object["pos"].tolist()] + [
                                            filtered_object["heading"]],
                                        "vel": [round(x, 4) for x in filtered_object["vel"].tolist()]
                                    }
                                    with self.serialLock:
                                        self.ser.write(
                                            f"{filtered_object['droneIndex']}{json.dumps(serial_data)}".encode('utf-8'))
                                        time.sleep(0.001)
                    ...
                    self.socketio.emit("object-points", {
                        ...
                        "objects": [{k: (v.tolist() if isinstance(v, np.ndarray) else v) for (k, v) in object.items()}
    ...
    for i in range(1, len(camera_poses)):
        ...
        for j, [a, b, c] in enumerate(epipolar_lines):
            ...
            if len(possible_matches) == 0:
                ...
                not_closest_match_image_points = [row for row in not_closest_match_image_points.tolist() if
                                                  row != possible_matches.tolist()[0]]
        ...
        if np.sum(num_matches) >= 2:
            for possible_pair in cartesian_product(matches_index, matches_index):
                pair_distance = np.sqrt(
                    np.sum((object_points[possible_pair[0]] - object_points[possible_pair[1]]) ** 2))
...
def add_white_border(image, border_size):
    ...
    bordered_image = cv.copyMakeBorder(image, border_size, border_size, border_size, border_size, cv.BORDER_CONSTANT,
                                       value=[255, 255, 255])
